...
void showScript() {
  ...
  var choose = int.tryParse(string);
  switch (choose) {
    case 1:
...
void showMatrixMap() {
  List<Map<String, String>> matrixMap = [
    {'thang': 'deptrai', 'passion': 'football'},
    {'quang': 'deptrai', 'passion': 'football'},
    {'tai': 'deptrai', 'passion': 'football'},
    {'thinh': 'deptrai', 'passion': 'football'},
    {'thánh': 'deptrai', 'passion': 'football'},
  ];
  print("Thắng ${matrixMap[0]['thang']}");
  print("Đam mê " + matrixMap[0]['passion']);
}

void showJsonMap() {
  Map<String, String> jsonMap = {'Name': 'thangdeptrai', 'Age': '26'};
  print("Name : ${jsonMap['Name']}");
  print("Tuổi : " + jsonMap['Age']);
}
...
void sortAscInArray(List<int> arr) {
  return arr.sort();
}
...
int binarySearchInArray(List<int> arr, int n, int giaTri) {
  int r = n - 1; // chỉ số phần tử cuối
  int l = 0; // Chỉ số phần tử đầu tiên
  while (r >= l) {
    int mid = l +
        (r - l) ~/
            2; // Tương đương (l+r)/2 nhưng ưu điểm tránh tràn số khi l,r lớn
...
void showSearchInArray(List<int> arr, String searchType, int giaTri) {
  ...
  switch (searchType) {
    ...
    case "binarySearchElement":
      int check = binarySearchInArray(arr, n, giaTri);
      if (check == -1) {
        print("Không tìm thấy $giaTri trong mảng");
      } else {
        print("Tìm thấy $giaTri trong mảng");
